...
setwd("F:/School/SeagrassRecovery/Data/HOBO/Temperature")
...
central_rsq_label <- paste("R^2 == ", round(summary(central_lm)$r.squared,3))
northern_rsq_label <- paste("R^2 == ", round(summary(northern_lm)$r.squared,3))
...
central_plot <-
  ...
  annotate("text", x = 1, y = 35,
           label = 'Slope = 1.005 ± 0.000',
           size = 5, fontface = 1, hjust = 0) +
  annotate("text", x = 1, y = 33,
           label = central_rsq_label, parse = TRUE,
           size = 5, fontface = 1, hjust = 0) +
  theme_bw() +
...
northern_plot <-
  ...
  annotate("text", x = 1, y = 35,
           label = 'Slope = 0.998 ± 0.000',
           size = 5, fontface = 1, hjust = 0) +
  annotate("text", x = 1, y = 33,
           label = northern_rsq_label, parse = TRUE,
           size = 5, fontface = 1, hjust = 0) +
  theme_bw() +
  theme(panel.grid = element_blank(),
        text = element_text(size = 16),
...
control_rsq_label <- paste("R^2 == ", round(summary(control_lm)$r.squared,3))
treatment_rsq_label <- paste("R^2 == ", round(summary(treatment_lm)$r.squared,3))
...
control_plot <-
  ...
  labs(x = expression(paste("Central Control Sed. Temp. (", degree, "C)")),
       y = expression(paste("Northern Control Sed. Temp. (  ", degree, "C)"))) +
  annotate("text", x = 1, y = 35,
           label = 'Slope = 0.952 ± 0.001',
           size = 5, fontface = 1, hjust = 0) +
  annotate("text", x = 1, y = 33,
           label = control_rsq_label, parse = TRUE,
           size = 5, fontface = 1, hjust = 0) +
  theme_bw() +
  theme(panel.grid = element_blank(),
        text = element_text(size = 16),
...
treatment_plot <-
  ...
  annotate("text", x = 1, y = 35,
           label = 'Slope = 0.935 ± 0.002',
           size = 5, fontface = 1, hjust = 0) +
  annotate("text", x = 1, y = 33,
           label = treatment_rsq_label, parse = TRUE,
           size = 5, fontface = 1, hjust = 0) +
  theme_bw() +
  theme(panel.grid = element_blank(),
        text = element_text(size = 16),
...
spatial_rsq_label <- paste("R^2 == ", round(summary(spatial_lm)$r.squared,3))
...
ggplot(data = dat_hourly, aes(x = Central, y = Northern)) +
  ...
  annotate("text", x = 1, y = 35,
           label = 'Slope = 0.948 ± 0.000',
           size = 5, fontface = 1, hjust = 0) +
  annotate("text", x = 1, y = 33,
           label = spatial_rsq_label, parse = TRUE,
           size = 5, fontface = 1, hjust = 0) +
  theme_bw() +
  theme(panel.grid = element_blank(),
        text = element_text(size = 16),
...
ggplot(data = dat_daily) +
  ...
  geom_line(aes(x = date, y = MeanNorth), size = 1) +
...
ggplot(data = dat_daily) +
  ...
  annotate("rect", fill = "black", alpha = 0.15, 
           ...
           xmax = as.Date("2021-10-01", "%Y-%m-%d"),
           ymin = -Inf, ymax = Inf) +
  annotate("text", x = as.Date("2020-07-01", "%Y-%m-%d"), y = 3,
           label = 'Pos. = Central > North',
           size = 5, fontface = 1, hjust = 0) +
  annotate("text", x = as.Date("2020-07-01", "%Y-%m-%d"), y = 2.8,
...
### 1989-2021 Wachapreague water level wrangling

wl_files <- list.files("F:/School/SeagrassRecovery/Data/Wachapreague/WaterLevel",full.names=T)
...
wl_dat_2021 <- wl_dat[262993:270192,]
wl_dat_1989_2020$date <- as.Date(wl_dat_1989_2020$date, '%Y/%m/%d')
wl_dat_2021$date <- as.Date(wl_dat_2021$date, '%m/%d/%Y')
wl_dat_1989_2020$hour <- as.numeric(substr(wl_dat_1989_2020$time,1,nchar(wl_dat_1989_2020$time)-3))
wl_dat_2021$hour <- as.numeric(substr(wl_dat_2021$time,1,nchar(wl_dat_2021$time)-3))
wl_dat <- rbind(wl_dat_1989_2020,wl_dat_2021)
...
### 1994-2021 Wachapreague water temp wrangling

wt_files <- list.files("F:/School/SeagrassRecovery/Data/Wachapreague/WaterTemp",full.names=T)
...
wt_dat_2021 <- wt_dat[216409:223608,]
wt_dat_1994_2020$date <- as.Date(wt_dat_1994_2020$date, '%Y/%m/%d')
wt_dat_2021$date <- as.Date(wt_dat_2021$date, '%m/%d/%Y')
wt_dat_1994_2020$hour <- as.numeric(substr(wt_dat_1994_2020$time,1,nchar(wt_dat_1994_2020$time)-3))
wt_dat_2021$hour <- as.numeric(substr(wt_dat_2021$time,1,nchar(wt_dat_2021$time)-3))
wt_dat <- rbind(wt_dat_1994_2020,wt_dat_2021)
...
ggplot(data = water_level_temp, aes(x = waterlevel_m, y = watertemp_c)) +
  geom_point(alpha = 0.25) +
  stat_smooth(method = "lm")

sed_wtemp_wlevel <- left_join(sed_dat,water_level_temp, by = c("date","hour"))
sed_wtemp_wlevel$doy <- yday(sed_wtemp_wlevel$date)
...
sed_wtemp_daily %>%
  ...
  geom_line(aes(x = date, y = deltaT_northern), color = "black", size = 1) +
...
train_central_lm_test <- lm(sedtemp_central~watertemp_c+waterlevel_m+doy+hour, data = Train)
train_northern_lm_test <- lm(sedtemp_northern~watertemp_c+waterlevel_m+doy+hour, data = Train)
summary(train_central_lm_test)
summary(train_northern_lm_test)

# test model fits using validation datasets
pred_Valid_central <- predict(train_central_lm_test, newdata = Valid)
pred_Valid_northern <- predict(train_northern_lm_test, newdata = Valid)
Valid$pred_central <- pred_Valid_central
Valid$pred_northern <- pred_Valid_northern
Valid_lm_central <- lm(pred_central~sedtemp_central, data = Valid)
Valid_lm_northern <- lm(pred_northern~sedtemp_northern, data = Valid)
summary(Valid_lm_central)
summary(Valid_lm_northern)

valid_central_rsq_label <- paste("R^2 == ", round(summary(Valid_lm_central)$r.squared,3))
valid_northern_rsq_label <- paste("R^2 == ", round(summary(Valid_lm_northern)$r.squared,3))

central_plot <-
  ggplot(data = Valid, aes(x = sedtemp_central, y = pred_central)) +
  geom_point(alpha = 0.25) +
  geom_abline(slope = 1, intercept = 0, linetype = 'longdash', color = "red") +
  stat_smooth(method = 'lm') +
  annotate("text", x = 1, y = 35,
           ...
           size = 5, fontface = 1, hjust = 0) +
  annotate("text", x = 1, y = 33,
           label = 'Slope = 0.983 ± 0.002',
           size = 5, fontface = 1, hjust = 0) +
  annotate("text", x = 1, y = 31,
           label = valid_central_rsq_label, parse = TRUE,
           size = 5, fontface = 1, hjust = 0) +
  scale_x_continuous(breaks = seq(0,35,5),
                     limits = c(0,35)) +
  scale_y_continuous(breaks = seq(0,35,5),
                     limits = c(0,35)) +
  labs(x = expression(paste("Observed Sed. Temp. ( ", degree, "C)")),
       y = expression(paste("Predicted Sed. Temp. ( ", degree, "C)"))) +
  theme_bw() +
  theme(panel.grid = element_blank(),
        text = element_text(size = 16),
        axis.text.x = element_text(size = 14, color = "black"),
...
northern_plot <-
  ggplot(data = Valid, aes(x = sedtemp_northern, y = pred_northern)) +
  geom_point(alpha = 0.25) +
  geom_abline(slope = 1, intercept = 0, linetype = 'longdash', color = "red") +
  stat_smooth(method = 'lm') +
  annotate("text", x = 1, y = 35,
           label = 'Northern', color = "red",
           size = 5, fontface = 1, hjust = 0) +
  annotate("text", x = 1, y = 33,
           label = 'Slope = 0.977 ± 0.002',
           size = 5, fontface = 1, hjust = 0) +
  annotate("text", x = 1, y = 31,
           label = valid_northern_rsq_label, parse = TRUE,
           size = 5, fontface = 1, hjust = 0) +
  scale_x_continuous(breaks = seq(0,35,5),
                     limits = c(0,35)) +
  scale_y_continuous(breaks = seq(0,35,5),
...
longterm_pred_central <- predict(train_central_lm_test, newdata = longterm_dat)
longterm_pred_northern <- predict(train_northern_lm_test, newdata = longterm_dat)
...
figD <- ggplot(data = water_sum, aes(x = Year, y = TotalDuration)) +
  ...
  scale_y_continuous(breaks = seq(0, 80, 10),
                     limits = c(0,80)) +
  ylab("Total MHW Days") +
  annotate("text", x = 1995, y = 80, label = "y = 0.62x - 1238", size = 5, hjust = 0) +
  annotate("text", x = 1995, y = 75,
           label = "paste(Adj.R ^ 2, \" = 0.14\")", parse = TRUE, size = 5, hjust = 0) +
  annotate("text", x = 1995, y = 70, label = "p-value = 0.033", size = 5, hjust = 0) +
...
figE <- ggplot(data = central_sum, aes(x = Year, y = TotalDuration)) +
  ...
  annotate("text", x = 1995, y = 80, label = "y = 0.81x - 1600", size = 5, hjust = 0) +
  annotate("text", x = 1995, y = 75,
           label = "paste(Adj.R ^ 2, \" = 0.15\")", parse = TRUE, size = 5, hjust = 0) +
  annotate("text", x = 1995, y = 70, label = "p-value = 0.032", size = 5, hjust = 0) +
  # annotate("text", x = 2019.5, y = 690, label = "(d", size = 6) +
  theme_bw() +
  theme(panel.grid = element_blank(),
        text = element_text(size = 16),
...
figF <- ggplot(data = northern_sum, aes(x = Year, y = TotalDuration)) +
  ...
  annotate("text", x = 1995, y = 80, label = "y = 0.79x - 1575", size = 5, hjust = 0) +
  annotate("text", x = 1995, y = 75,
           label = "paste(Adj.R ^ 2, \" = 0.14\")", parse = TRUE, size = 5, hjust = 0) +
  annotate("text", x = 1995, y = 70, label = "p-value = 0.039", size = 5, hjust = 0) +
  # annotate("text", x = 2019.5, y = 690, label = "(d", size = 6) +
  theme_bw() +
  theme(panel.grid = element_blank(),
        text = element_text(size = 16),
